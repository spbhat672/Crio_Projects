  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          variant="contained"
          onClick={() => addAddress(token, addressInput)}
        >
          Add
        </Button>
        <Button variant="text" 
...
const Checkout = () => {
  ...
  const getTotalQuantityOfProducts = ()=>{
    const totalQty = items.reduce((acc, item) => acc + parseInt(item.qty), 0);
    return totalQty;
  }
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      let addressObj = { address: newAddress };
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      // TODO: CRIO_TASK_MODULE_CHECKOUT - Display the latest list of addresses
      setAddresses((currAddresses) => ({
        ...currAddresses,
        all: response.data,
      }))
  ...
  // Helper function to get total cart amount
const getTotalAmount = (items) => {
  ...
  const validateRequest = (items, addresses) => {
    const cartTotalAmount = getTotalAmount(items);
    if (cartTotalAmount > localStorage.getItem("balance")) {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      const response = await axios.post(`${config.endpoint}/cart/checkout`,
        ...
        // checkout successful, return order ID
        enqueueSnackbar("Checkout sucess.", {
          variant: "success",
          });
          const updatedBalance = parseInt(localStorage.getItem("balance")) - getTotalAmount(items);
          localStorage.setItem("balance",updatedBalance);
          history.push("/thanks");
        } else {
        // checkout failed, throw error
        enqueueSnackbar("Failed to checkout.", {
          variant: "error",
          });
        }
    }catch(err){
    ...
    <>
      ...
      <Grid container>
              ...
              {addresses.all.length > 0 ? (
                ...
                <div>
                  {addresses.all.map((address) => {
                    ...
                    return (
                      <Box key={address._id} className={currentClassName} onClick={(event) =>
                        setAddresses((currAddresses) => ({
                          ...currAddresses,
                          selected: address,
                        }))
                ...
                // Display "No addresses found" message
                <Typography my="1rem">
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          <Cart style={{backgroundColor:"white"}} isReadOnly products={products} items={items} />
          <Box>
            <h2>Order Details</h2>
            <div>Products {getTotalQuantityOfProducts()}</div>
            <div>Subtotal ${getTotalAmount(items)}</div>
            <div>Shipping Charges $0</div>
            <div><h2>Total ${getTotalAmount(items)}</h2></div>
          </Box>
        </Grid>
      </Grid>
      <Footer />
